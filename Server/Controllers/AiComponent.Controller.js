...
import { askAi } from "../Utils/openRoute.js";

export const generateComponentCode = async (req, res) => {
    ...
    try {
        ...
        if (user.role === "user") {
            ...
            // 'credits' ki jagah 'aiCredits' use karein (jo apke schema me ho)
            if (user.aiCredits < 50) { // '<=' ki jagah '<' lagayein
                return res.status(403).json({
                    message: "Not enough credits"
                });
            }

            user.aiCredits -= 50; // Yahan aiCredits minus karein
            await user.save();
        }
